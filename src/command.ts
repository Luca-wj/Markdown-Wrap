...
export function insertTable(){
    ...
    const snippet = judgeNewLine(line) + "| ${1:head1} | ${2:head2} |\n| ----- | ----- |\n| ${3:body1} | ${4:body2} |";
...
export function insertCodeBlock() {
    const editor = window.activeTextEditor!;
    return editor.insertSnippet(new SnippetString(judgeNewLine(editor.selection.active.line) + '```$0\n$TM_SELECTED_TEXT\n```'));
}
...
export function insertMathBlock() {
    const editor = window.activeTextEditor!;
    return editor.insertSnippet(new SnippetString(judgeNewLine(editor.selection.active.line) + '\$\$\n$TM_SELECTED_TEXT$0\n\$\$'));
}
        ...
        for (let line = startLine; line <= endLine; line++) {
            ...
            if ((res !== null) && isQuote) {
                ...
                if (startLine === line && startLine !== 0) {
                    ...
                    batchEdit.insert(editor.document.uri, new Position(line, 0), `${judgeNewLine(startLine)}> `);
                } else {
...
function judgeNewLine(line: number) {
    const editor = window.activeTextEditor!;
    const doc = editor.document;
    const preLine = line - 1 > 0? line - 1: 0;
    return doc.lineAt(preLine).text.trim() === ''? '': '\n';
}